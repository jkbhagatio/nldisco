...
class EvaluationCallback(pl.Callback):
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        rates = rates.exp().detach().cpu().numpy()
        ...
        *_, train_behavior = trainer.datamodule.train_data
        ...
        train_rates = [
            self.batch_fwd(pl_module, batch)[0].detach().cpu() for batch in train_dataloader
        ]
        train_rates = torch.cat(train_rates)
        train_rates=train_rates.exp().numpy()
...
class LorentzEvaluationCallback(pl.Callback):
    """Computes and logs all evaluation metrics for Lorentz atttractor.
    """
    def __init__(
        self, batch_fwd=batch_fwd, log_every_n_epochs=20, decoding_cv_sweep=False
    ):
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        recon_data = recon_data.detach().cpu().view(recon_data.size(0)*recon_data.size(1),recon_data.size(2)).numpy()
        # Get model predictions for the entire validation dataset
        val_dataloader = trainer.datamodule.val_dataloader()
        # Pass the data through the model
        rates = [self.batch_fwd(pl_module, batch)[0].detach().cpu() for batch in val_dataloader]
        rates = torch.cat(rates)
        rates = rates.exp()
        rates = rates.view(rates.size(0)*rates.size(1),rates.size(2)).numpy()
        r2 = r2_score(rates,recon_data, multioutput='uniform_average')
        pl_module.log("nlb/r2", max(r2, -1.0))
