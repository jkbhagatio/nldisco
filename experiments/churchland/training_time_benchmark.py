...
def report(output: Path) -> None:
    ...
    for method, name in zip(METHODS, NAMES):
        ...
        totals = {r["parameter_count"] for r in group}
        if len(totals) != 1:
            raise ValueError(f"Parameter counts differ across {method} fits")
        total = totals.pop()
        if all("trainable_parameter_count" in r for r in group):
            trainable_counts = {r["trainable_parameter_count"] for r in group}
        else:
            audit = json.loads((output / f"parameter_audit_{method}.json").read_text())
            if audit["total"] != total:
                raise ValueError(f"Parameter audit disagrees with {method} fits")
            trainable_counts = {audit["trainable"]}
        if len(trainable_counts) != 1 or not 0 <= min(trainable_counts) <= total:
            raise ValueError(f"Invalid trainable-parameter counts for {method}")
        parameter_descriptions.append(f"{name}, {total:,}/{trainable_counts.pop():,}")
    ...
    ax.set(xticks=[1, 2, 3], xticklabels=NAMES, ylabel="Training time (s)",
           title="LVM training time comparison",
           ylim=(0, 1.15 * max(r["train_seconds"] for r in rows)))
    ax.spines[["top", "right"]].set_visible(False)
    ax.yaxis.grid(True, alpha=.18)
    ax.set_axisbelow(True)
    fig.savefig(output / "training_times.pdf", bbox_inches="tight")
    fig.savefig(output / "training_times.png", dpi=220, bbox_inches="tight")
    plt.close(fig)
...
\begin{figure}[htbp]
    ...
    PARAMETER_COUNTS. Each dot is one fresh fit; five seeds (0--4) per method
    ...
    latent export, and NLDisco threshold calibration (median CALIBRATION_SECONDS~s).
...
"""
    ...
    appendix = appendix.replace("PARAMETER_COUNTS", "; ".join(parameter_descriptions))
