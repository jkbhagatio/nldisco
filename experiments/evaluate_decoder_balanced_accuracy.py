...
def churchland(output: Path) -> None:
    ...
    records = json.loads((DEFAULT_OUTPUT / "churchland/results.json").read_text())["results"]
    ...
    for record in records:
        ...
        prediction_path = path.with_name(path.name.replace("_probe.joblib", "_predictions.npz"))
        ...
        row = dict(key=key, method=method, feature=feature, space=space, latent_id=column,
                   ...
                   model_sha256=decoder.sha256(path), source_test_auroc=record["source_test_auroc"])
        tuned.append(row)
