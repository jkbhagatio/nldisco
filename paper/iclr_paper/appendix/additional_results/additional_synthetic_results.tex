...
To measure a latent's preference for forward or backward movement, we compare its mean activation in the two directions:
\begin{equation}
\mathrm{DI}=
\frac{\bar z_{\mathrm{forward}}-\bar z_{\mathrm{backward}}}
{\bar z_{\mathrm{forward}}+\bar z_{\mathrm{backward}}+10^{-12}}.
\end{equation}
DI ranges from $-1$ to $1$: positive values indicate greater forward activation, negative values greater backward activation, and zero equal activation. Here, $\bar z_{\mathrm{forward}}$ and $\bar z_{\mathrm{backward}}$ are mean activations after matching the distribution of mean window position across directions, including windows in which the latent is inactive.
...
\paragraph{Temporal feature selection.}
We apply the same selection criteria to FW-SED and TW-SED latents, assessing each movement direction separately. A passing latent must have a clean-crossing DI of at least 0.25 in the tested direction, decoder-center displacement of at least 0.01 m in that direction, and at least 20 active clean-crossing windows across both directions, with at least two matched position bins.

To measure decoder-center displacement, we weight the known centers of cells 1 and 2 by their positive squared decoder weights, then subtract the weighted center over the first six timebins from that over the last six. The pair's mean positive squared weight per entry must be at least twice that of the 96 noise units. Each cell must contribute at least 10\% of the pair's total positive squared weight, and the pair at least 50\% of that across all four place cells. We rank passing latents by the product of direction-signed DI, direction-signed displacement, and $\log(1+\mathrm{enrichment})$, where enrichment is the pair-to-noise ratio above. Decoder displacement describes a learned pattern, not a calibrated trajectory estimate.
...
\midrule
20 (forward) & 0 & $+0.558$ & $+0.118$ & $15/16$ & $16/16$ \\
66 (backward) & 2 & $-0.479$ & $-0.199$ & $13/16$ & $16/16$ \\
\bottomrule
\end{tabular}
\end{center}
